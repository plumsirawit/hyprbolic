...
COverview::COverview(PHLWORKSPACE startedOn_, bool swipe_) : startedOn(startedOn_), swipe(swipe_) {
    ...
    static auto* const* PCOLUMNS = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprbolic:columns")->getDataStaticPtr();
    static auto* const* PGAPS    = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprbolic:gap_size")->getDataStaticPtr();
    static auto* const* PCOL     = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprbolic:bg_col")->getDataStaticPtr();
    static auto* const* PSKIP    = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprbolic:skip_empty")->getDataStaticPtr();
    static auto const*  PMETHOD  = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprbolic:workspace_method")->getDataStaticPtr();
    ...
    static const GLfloat g_buf_data[] = {
        -1.0f, -1.0f,
        1.0f, -1.0f,
        -1.0f, 1.0f,
        1.0f, 1.0f
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(g_buf_data), g_buf_data, GL_STATIC_DRAW);    
}
...
void COverview::fullRender() {
    ...
    GLuint programID = LoadShaders( "/home/sirawit/Documents/hyprbolic/shaders/HyperbolicVertexShader.glsl", "/home/sirawit/Documents/hyprbolic/shaders/HyperbolicFragmentShder.glsl" );
    glUseProgram(programID);
    GLuint u_resolution_loc = glGetUniformLocation(programID, "u_resolution");
    GLuint u_center_loc = glGetUniformLocation(programID, "u_center");
    glUniform2f(u_resolution_loc, SIZE.x, SIZE.y);
    glUniform2f(u_center_loc, 0.0, 0.0);
...
void COverview::onSwipeUpdate(double delta) {
    ...
    static auto* const* PDISTANCE = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprbolic:gesture_distance")->getDataStaticPtr();
