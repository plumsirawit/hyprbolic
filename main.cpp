...
const std::string KEYWORD_BOLIC_GESTURE = "hyprbolic-gesture";
...
static SDispatchResult onBolicDispatcher(std::string arg) {
...
static void failNotif(const std::string& reason) {
    HyprlandAPI::addNotification(PHANDLE, "[hyprbolic] Failure in initialization: " + reason, CHyprColor{1.0, 0.2, 0.2, 1.0}, 5000);
}

static Hyprlang::CParseResult bolicGestureKeyword(const char* LHS, const char* RHS) {
    ...
    for (const auto arg : std::string(LHS).substr(KEYWORD_BOLIC_GESTURE.size())) {
        switch (arg) {
            case 'p': disableInhibit = true; break;
            default: result.setError("hyprbolic-gesture: invalid flag"); return result;
        }
    }
    ...
    if (data[startDataIdx] == "bolic")
        resultFromGesture = g_pTrackpadGestures->addGesture(makeUnique<CBolicGesture>(), fingerCount, direction, modMask, deltaScale, disableInhibit);
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    HyprlandAPI::addDispatcherV2(PHANDLE, "hyprbolic:bolic", ::onBolicDispatcher);

    HyprlandAPI::addConfigKeyword(PHANDLE, KEYWORD_BOLIC_GESTURE, ::bolicGestureKeyword, {true});

    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprbolic:columns", Hyprlang::INT{3});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprbolic:gap_size", Hyprlang::INT{5});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprbolic:bg_col", Hyprlang::INT{0xFF111111});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprbolic:workspace_method", Hyprlang::STRING{"center current"});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprbolic:skip_empty", Hyprlang::INT{0});

    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprbolic:gesture_distance", Hyprlang::INT{200});

    HyprlandAPI::reloadConfig();

    return {"hyprbolic", "A plugin for an overview", "Vaxry", "1.0"};
}
